...
export function NewConversation({ agentInstanceId }: NewConversationProps) {
  const { dataSetId } = useApplicationContext()!;
  const navigate = useNavigate();

  const onSubmit = async (message: string) => {
    const newConversationId = await api.conversations().createConversation(agentInstanceId);

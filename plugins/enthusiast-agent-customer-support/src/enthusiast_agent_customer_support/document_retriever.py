...
class ManualsDocumentRetriever(BaseVectorStoreRetriever[DocumentChunk]):
    def find_content_matching_query(self, query: str, product_type: str) -> QuerySet[DocumentChunk]:
        embedding_vector = self._create_embedding_for_query(query)
        relevant_documents = self._find_documents_matching_vector(embedding_vector, product_type)
    ...
    def _find_documents_matching_vector(
        self, embedding_vector: list[float], product_type: str
        ...
        ).filter(document__title=f"{product_type}.pdf")

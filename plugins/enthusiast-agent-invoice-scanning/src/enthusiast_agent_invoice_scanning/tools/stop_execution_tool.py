...
class StopExecutionTool(BaseLLMTool):
    ...
    Use this ONLY when further progress is impossible — for example when no
    eCommerce connector is configured, or when the invoice contains no recognisable
    line items.

    Do NOT call this for retryable failures. Provide a concise ``stop_reason`` that
    explains why the execution must stop; it will be surfaced as the execution failure
    summary.
    """
    ...
    DESCRIPTION = (
        ...
        "Call this ONLY when further progress is impossible — for example when no "
        "eCommerce connector is configured, or when the invoice contains no recognisable "
        "line items. "

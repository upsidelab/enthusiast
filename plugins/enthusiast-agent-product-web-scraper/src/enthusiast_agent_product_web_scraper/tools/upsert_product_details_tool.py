...
class UpsertProductDetailsInput(BaseModel):
    """Input schema for a single product to upsert."""
    ...
    property_values_by_property_name_as_json: Optional[str] = Field(
        default=None,
        description="""
...
class UpsertProductBatchInput(BaseModel):
    """Input schema for a batch of products to upsert."""

    products: List[UpsertProductDetailsInput] = Field(description="List of products to upsert")


class UpsertProductDetailsTool(BaseLLMTool):
    """Creates or updates products in the configured ecommerce platform.

    Accepts a batch of products identified by SKU. For each product, checks whether it
    already exists and either updates or creates it accordingly.
    """
    ...
    def run(self, products: List[UpsertProductDetailsInput]) -> str:
        """Upsert a batch of products into the ecommerce platform.

        Args:
            products: List of products to create or update.

        Returns:
            JSON string mapping each SKU to a success or failure message.
        """
        ...
        for product in products:
            ...
            try:
                ...
                if product_exists:
                    product_upsert_result = self._update_product_details(
                        ecommerce_platform_connector, product_sku, product_details
                    )
                else:
                    product_upsert_result = self._create_product(
                        ecommerce_platform_connector, product_sku, product_details
                    )

...
class OllamaPrepareFileMapper:
    @staticmethod
    def gpt_image(file_object: LLMFile, data_placeholder: bool = True) -> OllamaFileObject:
        if data_placeholder:
            image_url = (
    ...
    @staticmethod
    def gpt_file(file_object: LLMFile, data_placeholder: bool = True) -> OllamaFileObject:
        if data_placeholder:
    ...
    @staticmethod
    def mistral_image(file_object: LLMFile, data_placeholder: bool = True) -> OllamaFileObject:
        if data_placeholder:
            image_url = (
                f"data:{file_object.content_type};base64,{{{LanguageModelProvider.FILE_KEY_PREFIX}{file_object.id}}}"
            )
    ...
    @staticmethod
    def mistral_file(file_object: LLMFile, data_placeholder: bool = True) -> OllamaFileObject:
        if data_placeholder:
            document_url = (
                f"data:{file_object.content_type};base64,{{{LanguageModelProvider.FILE_KEY_PREFIX}{file_object.id}}}"
            )
    ...
    @staticmethod
    def gemini_image(file_object: LLMFile, data_placeholder: bool = True) -> OllamaFileObject:
        if data_placeholder:
            image_url = (
                f"data:{file_object.content_type};base64,{{{LanguageModelProvider.FILE_KEY_PREFIX}{file_object.id}}}"
            )
    ...
    @staticmethod
    def gemini_file(file_object: LLMFile, data_placeholder: bool = True) -> OllamaFileObject:
        if data_placeholder:
    ...
    MODEL_MAPPING: dict[str, dict[str, Callable[[LLMFile, bool], OllamaFileObject]]] = {
        "gpt": {
    ...
    def get_prepare_file_function(self, model: str, file_type: str) -> Callable[[LLMFile, bool], OllamaFileObject]:
...
class OllamaLanguageModelProvider(LanguageModelProvider):
    ...
    def prepare_image_object(self, file_object: LLMFile, data_placeholder: bool = True) -> OllamaFileObject:
        return self.MODEL_MAPPING_CLASS().get_prepare_file_function(model=self._model, file_type="image")(
            file_object, data_placeholder
        )

    def prepare_file_object(self, file_object: LLMFile, data_placeholder: bool = True) -> OllamaFileObject:
        return self.MODEL_MAPPING_CLASS().get_prepare_file_function(model=self._model, file_type="file")(
            file_object, data_placeholder
        )

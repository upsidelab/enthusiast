...
class ShopwareProductSource(ProductSourcePlugin):
    ...
    def get_product(self, product_details):
        ...
        return ProductDetails(
            ...
            properties=self.product_properties(product_details),
            categories=str([self.product_category(category_id) for category_id in
                            product_details.get("attributes", {}).get("categoryTree")])
        )
    ...
    def fetch_products(self):
        response = requests.get(f'{self._base_url}/api/product', headers=self.build_headers())
        if response.status_code != 200:
        ...
        data = filter(lambda o: o.get('type') == 'product' and
                                o.get('attributes', {}).get('productNumber') and
                                '.' not in o.get('attributes', {}).get('productNumber'),
                      data)
        return list(data)
    ...
    def fetch_categories(self):
        ...
        response = requests.get(f'{self._base_url}/api/category', headers=self.build_headers())
        if response.status_code != 200:
    ...
    def fetch_currency_id(self):
        if self._currency_id:
            return self._currency_id

        response = requests.get(f'{self._base_url}/api/currency', headers=self.build_headers())
        if response.status_code != 200:
            raise Exception("Failed to fetch currency id")

        data = response.json().get('data', [])
        self._currency_id = next(
            (c.get('id') for c in data if c.get('attributes', {}).get('isoCode') == self._currency_iso_code),
            None
        )
    ...
    def fetch_properties(self):
        ...
        response = requests.get(f'{self._base_url}/api/property-group', headers=self.build_headers())
        if response.status_code != 200:
            raise Exception("Failed to fetch properties")

        data = response.json().get('data', [])
        self._properties = {
            property_data.get('id'): property_data.get('attributes', {}).get('name')
            for property_data in data
        }

        return self._properties

    def fetch_property_options(self):
        if self._property_options:
            return self._property_options

        response = requests.get(f'{self._base_url}/api/property-group-option', headers=self.build_headers())
        if response.status_code != 200:
            raise Exception("Failed to fetch property options")

        data = response.json().get('data', [])
        self._property_options = {
            property_option_data.get('id'): { 'name': property_option_data.get('attributes', {}).get('name'),
    ...
    def product_category(self, category_id):
        return self.fetch_categories().get(category_id)

    def product_price(self, product_details):
        price = next((price.get('gross') for price in product_details.get('attributes', {}).get('price') if
                      price.get('currencyId') == self.fetch_currency_id()), None)
        return float(price) if price else None

    def product_properties(self, product_details):
        ...
        for property_option_id in product_details.get('attributes', {}).get('propertyIds'):
            property_option = self.fetch_property_options().get(property_option_id)
            property_name = self.fetch_properties().get(property_option.get('property_id'))
        ...
        return ' | '.join([f'{property_name} -> {str(property_options)}'
    ...
    def fetch(self) -> list[ProductDetails]:
        ...
        """
        products = []
        for product_details in self.fetch_products():
            products.append(self.get_product(product_details))

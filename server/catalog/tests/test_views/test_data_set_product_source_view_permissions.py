...
@pytest.fixture
def detail_url(data_set, product_source):
    return reverse(
        "data_set_product_source_details", kwargs={"data_set_id": data_set.id, "product_source_id": product_source.id}
    )
...
class TestDataSetProductSourceListViewGet:
    ...
    def test_user_with_view_permission_only_sees_sources_from_their_dataset(
        ...
        baker.make(ProductSource, data_set=data_set, _quantity=2)
        baker.make(ProductSource, data_set=other_dataset, _quantity=3)
...
class TestDataSetProductSourceListViewPost:
    def test_user_with_change_permission_can_create_source(
        ...
        assert ProductSource.objects.filter(data_set=data_set, plugin_name="Test Source").exists()

    def test_user_without_change_permission_cannot_create_source(self, api_client, url, payload, data_set):
        response = api_client.post(url, payload, format="json")

        assert response.status_code == status.HTTP_403_FORBIDDEN
        assert not ProductSource.objects.filter(data_set=data_set, plugin_name="Test Source").exists()
...
class TestDataSetProductSourceViewGet:
    def test_user_with_view_permission_can_get_source(
        self, api_client_with_view_permission, detail_url, product_source
    ):
    ...
    def test_user_without_permission_cannot_get_source(self, api_client, detail_url, product_source):
...
class TestDataSetProductSourceViewPatch:
    ...
    def test_user_with_change_permission_can_update_source(
        self, api_client_with_change_permission, detail_url, payload, product_source
    ):
        response = api_client_with_change_permission.patch(detail_url, payload, format="json")

        assert response.status_code == status.HTTP_200_OK
        product_source.refresh_from_db()
        assert product_source.plugin_name == "Updated Source"

    def test_user_without_change_permission_cannot_update_source(self, api_client, detail_url, payload, product_source):
        response = api_client.patch(detail_url, payload, format="json")

        assert response.status_code == status.HTTP_403_FORBIDDEN


class TestDataSetProductSourceViewDelete:
    def test_user_with_delete_permission_can_delete_source(
        self, api_client_with_change_permission, detail_url, product_source
    ):
        ...
        assert not ProductSource.objects.filter(id=source_id).exists()

    def test_user_without_delete_permission_cannot_delete_source(self, api_client, detail_url, product_source):
        source_id = product_source.id

        response = api_client.delete(detail_url)

        assert response.status_code == status.HTTP_403_FORBIDDEN
        assert ProductSource.objects.filter(id=source_id).exists()

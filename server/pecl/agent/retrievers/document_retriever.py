...
from ecl.models import DocumentChunkEmbedding, DocumentChunk, DataSet, EmbeddingModel, EmbeddingDimension
...
class DocumentRetriever:
    ...
    def _find_documents_matching_vector(self, embedding_vector: list[float]) -> list[DocumentChunk]:
        ...
        embeddings_by_distance = DocumentChunkEmbedding.objects.annotate(distance=embedding_distance).order_by("distance")
        embeddings_with_documents = embeddings_by_distance.select_related('chunk__document').filter(
            model__exact=self.embedding_model,
            dimensions__exact=self.embedding_dimensions,
            chunk__document__data_set_id__exact=self.data_set.id)
        limited_embeddings_with_documents = embeddings_with_documents[:self.max_documents]

        return list(map(lambda x: x.chunk, limited_embeddings_with_documents))
